...
| 来源 | 作用 |
|------|------|
| `workshopcache.json` | 订阅列表的权威来源，标题与标注大小取自这里 |
| Steam 的 `appworkshop_431960.acf` | 交叉校验：`WorkshopItemsInstalled` 里有记录的目录一律不清理。它由 Steam 写在自己库里（`steamapps\workshop\` 下），程序只读它，**不需要 Steam 正在运行** |
| 目录自身的修改时间 | 兜底：30 分钟内被改动过的目录先不删，避免碰到还在下载的壁纸 |

第二路是为了修掉一个真实问题：Steam 下载时会立刻建好内容目录，而 Wallpaper Engine 只在它自己刷新工坊列表时才改写 `workshopcache.json`，实测可能相差几十分钟。窗口期内只看缓存，就会把刚下载完的壁纸显示成「已取消订阅」并自动勾选。

这两个额外来源只用于**保护**，不会把任何目录变成新的待清理项；读不到（文件不存在、格式变化、Steam 正在重写）就静默降级为只看缓存，不影响原有功能。
...
在交叉核对（Steam 安装记录）与新鲜度保护之后，剩余的文件夹分成两类，避免误删：
...
- 程序仅对比订阅列表、Steam 安装记录与本地文件，不会删除已订阅的壁纸
- 删除前会重新读一次订阅缓存与 Steam 安装记录：仍处于订阅 / 已安装状态的会被自动跳过，并在日志里说明原因
- 删除前还会跳过 30 分钟内被改动过的目录，避免删到正在下载的壁纸（面板会提示「跳过」，日志可查）
