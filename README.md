...
- **待清理列表**：列出所有不在订阅列表中的文件夹，带标题、类型与预览缩略图（点图可放大）。打开时已自动勾选「已取消订阅」的部分，想保留哪个取消勾选即可
- **点标题打开目录**：两张列表的标题都可以点，用系统文件管理器（Windows 资源管理器）打开这张壁纸所在的文件夹，方便删除前先看一眼里面到底是什么；没有标题的行显示 `—`，不可点
- **删除到回收站**：默认把文件夹移入回收站而不是永久删除，误操作可以从回收站恢复
- **已订阅列表**：查看订阅中的壁纸标题、预览图、标注大小与实际占用，支持按 ID / 标题筛选
